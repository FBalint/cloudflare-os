 ...
 * hide the read from the observers named here, `discard` when it did not.
 *
 ...
 * fence only has to reach a concurrent admission, and a DO runs in one isolate at a time. Keyed by
 * storage, as `credential-expiry.ts` requires, so per-session trackers share one count. A crash
 * strands the count until the isolate recycles, over-fencing rather than opening.
 */
const withholdsInFlight = new WeakMap<ObserverKv, number>();
...
/**
 * How many observers a binding may retain. Each one costs a verifier call per read, and Workers
 * cap a request at 32 Worker invocations — past that every read on the binding throws. Refusing
 * the admission is the legible half of that failure.
 */
const DEFAULT_MAX_OBSERVERS = 20;
...
export class ObserverTracker<V> {
  ...
  async addObserver(id: string, verifier: V): Promise<void> {
    ...
    if (kv.get<boolean>(OBSERVER_WITHHELD_KEY) || (withholdsInFlight.get(kv) ?? 0) > 0) {
  ...
  prepareWithheld(): ObservationCheck {
    ...
    const release = () => {
      if (settled) return;
      settled = true;
      withholdsInFlight.set(kv, (withholdsInFlight.get(kv) ?? 1) - 1);
    };

    withholdsInFlight.set(kv, (withholdsInFlight.get(kv) ?? 0) + 1);
    return {
  ...
  async prepareObservation(setIds: readonly string[]): Promise<ObservationCheck> {
    ...
    // Both partitions are computed before the first await, so the "pending" writes below reflect
    // storage as a concurrent addObserver will scan it.
    const promote = canonical.filter(setId => !this.#isObserved(setId));
    const untracked = canonical.filter(setId => this.#state(setId) === undefined);

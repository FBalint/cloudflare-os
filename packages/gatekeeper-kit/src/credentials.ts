...
const OWNED_KEYS: readonly string[] = [CREDENTIALS_KEY, IDENTITY_KEY, MIGRATED_KEY];
...
export type CredentialCoordinatorOptions<Creds> = {
  ...
  /**
   * One-shot migration off those keys, reassembling the grant they hold.
   *
...
export class CredentialCoordinator<Creds> {
   ...
   */
  async #coalesced(current: Creds, refresh: (current: Creds) => Promise<Creds>): Promise<Creds> {
 ...
 * mean a stale derived bearer rather than a dead grant should wrap its calls in `withAuthRetry`
 * from `./auth-retry` and report expiry only from `onPersistentAuthError`.
 */
export class CredentialSource<Creds> {
  ...
  async #current(): Promise<CredentialsWithIdentity<Creds>> {
    const fetching = (this.#fetching ??= this.#options.account().getCredentials());
    try {
  ...
  #invalidate(): void {
    // The in-flight fetch was started against the credentials just reported dead. Leaving it would
    // hand them to the next caller anyway.

...
abstract class BufferedCursor<T> extends RpcTarget implements Cursor<T> {
  ...
  /** Buffers one provider page's visible items, sets `remoteExhausted`, and answers how many. */
  protected abstract [loadMore](): Promise<number>;
...
export class PageNumberCursor<T> extends BufferedCursor<T> {
  ...
  constructor(options: PageNumberCursorOptions<T>) {
    super(options.pageSize);
    this.#fetchPage = options.fetchPage;
    this.#retain = options.retain;
    this.#remotePerPage = requirePositiveInt("remotePageSize", options.remotePageSize ?? 100);
  }

  protected override async [loadMore](): Promise<number> {
    const page = await this.#fetchPage(this.#remotePage, this.#remotePerPage);
...
export class TokenCursor<T> extends BufferedCursor<T> {
  ...
  protected override async [loadMore](): Promise<number> {
    ...
    const page = await this.#fetchPage(asked, this.#remotePerPage);

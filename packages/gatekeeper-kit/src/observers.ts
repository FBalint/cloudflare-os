...
export interface ObserverStrategy {
  ...
  observerIds?(): string[];
  /**
   * Reserves a read withheld from every observer. Present only where observers are retained;
   * absent means there is no list to withhold from.
   *
...
export class ObservationGate implements Disposable {
  ...
  /**
   * A strategy answering neither hook retains no observer list, which is then no exclusions: A
   * refuses admission outright, and B and D admit only observers who see everything read here.
   */
  async #prepare(scope: ObservationScope): Promise<ObservationCheck> {
    switch (scope.kind) {
      ...
      case "withholdFromObservers":
        return this.#strategy.prepareWithheld?.() ?? NOTHING_TO_RESOLVE;
      case "sets":

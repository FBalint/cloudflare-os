...
describe("ObservationGate", () => {
  ...
  it("withholds from nobody where the strategy retains no observer list", async () => {
    // B admits only observers who can read everything this binding reads, so there is nobody the
    // read has to be hidden from -- and no stored list to name them from either.
    const authorizeObservation = vi.fn(async () => {});

...
`packages/mcp-shared/package.json`. Four files are internal instead: `serial-queue` (§4.12) and the
two split out of `actions` and `observers`, each reached through its owning subpath, plus
`positive-int` — one `requirePositiveInt` shared by every module that takes a bound, since three
had begun to carry byte-identical copies.
...
export type ObserverTrackerOptions<V> = {
  ...
  maxObservers?: number;                // default 20; refuses to admit observer 21
...
export interface ObserverStrategy {
  ...
  prepareWithheld?(): ObservationCheck;   // enumerates and fences admission in one step
}
...
and invokes `commit()` after authorization succeeds or `discard?.()` when it refuses. Both hooks are
absent on A/B/D, which retain no list, so no exclusions: A refuses admission outright and B/D admit
only observers who see everything read here. Because the gate is now the only source of that field,
there is no set-union merge left to do. Sessions call the gate for every read instead of the raw
queue.
...
export function defineActions<H, M extends Record<string, unknown>>(defs: {
  [K in keyof M]: {
    ...
    provides?(payload: M[K]): Iterable<string>;    // provisional refs this payload creates
    dependsOn?(payload: M[K]): Iterable<string>;   // ... and those it needs an earlier one to create
...
| **`maxObservers` is a platform bound the corpus does not have.** Every retained observer costs one verifier call per read, and Workers cap a request at **32 Worker invocations** — past that the call throws, so a binding with too many collaborators fails *every* read rather than degrading. No shipped tracker caps this: notion, confluence, context, linear and internal `gatekeeper-shared` fan out over all observers with unbounded `Promise.all`, and google throttles concurrency without bounding the total. | every strategy-C port | The kit refuses at admission instead, which is the legible half of the same failure. 20 leaves headroom for `authorizeObservation` and the hops that reached the facet, but the real ceiling is per-deployment, so the number belongs in that port's options. `concurrency` is a throttle and never a bound. |
...
| **The retained tier is unbounded.** `#requireCapacity` scans only the pending prefix and skips `isRetained`, so `maxPending` bounds `staged`, `failed`, and pending records but never retained ones. A long-lived `retainApplied: true` binding accumulates one record per applied action indefinitely. | every retaining port | Retention is consumer policy and vendor caps differ; the binding must retire records through `runExclusive` under its own policy. |
...
**Only a record awaiting a user decision counts.** `staged` and `failed` are both excluded — one was
never delivered to the overseer, the other is cleared by rejecting it — and no journal in either tree
reclaims a record stranded between `allocate` and a successful `submitAction`, so counting either
would wedge `allocate` for a user with nothing in their approval queue to clear. But exclusion alone
would just move the growth: both sit under the *scanned* prefix, so a run of them would make every
later allocation and every simulation scan more expensive. The same `allocate` scan therefore drops
the oldest past the same bound — the newest is the one the user still has on screen — which keeps
that scan bounded by what it enforces.
...
One accepted edge, worth a line rather than machinery: pruning a `staged` record requires more than
`maxPending` concurrent in-flight submissions on one resource, and the oldest then loses its record,
its `markSubmitted` no-ops, and the action surfaces later as `Unknown pending action`.
...
silently drops the oldest records instead of nothing — worst at one below the cap, and invisible
at a cap of 2, the only value for which no prunable count reaches that band.
...
started before an `invalidateAll()` is handed to the caller that asked for it (which asked before
the change) and deliberately not written. Values live inside the generation that `invalidateAll()`
...
**Entries are partitioned by authority.** Every entry carries the `authorityId` its cache was
constructed with, and a read hits only when that matches, the generation matches, and the TTL has
...
**Concurrent misses coalesce.** An instance-local `Map` keyed `` `${generation}:${key}` `` holds the
in-flight load, so N callers missing one key run `load()` once, and the entry is cleared in a
`finally` only when it is still the same promise — the guard shape `CredentialSource` uses. The
generation is part of that key, so a load started before a bump is never shared with a caller that
arrived after it. Without this, the later-started load could store first and the earlier one
...
- `revoke()` — clears `"attemptGeneration"`, `deleteAlarm()` and `deleteAll()` **before** the first
  ...
  a connection begun during that await be erased by the revoke that preceded it.
...
arm; `credentials:identity` and `credentials:migrated` are the coordinator's identity fence and
migration marker. The original first four match every existing OAuth gatekeeper, so live accounts
keep working across a port.
